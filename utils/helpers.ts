...
/**
 * Check the status of a job offer based on its properties
 * @param offer - Job offer object to check
 * @returns string - Status of the offer ("accepted", "skipped", "expired", or "offered")
 */
export const checkOfferStatus = (offer: any): string => {
  ...
  if (typeof expiresAt === "number") {
    ...
    if (/^\d+$/.test(expiresAt.trim())) {
      expirationTime = Number(expiresAt);
    } else {
      const parsed = Date.parse(expiresAt);
      expirationTime = Number.isFinite(parsed) ? parsed : null;
    }
  }
  ...
  if (
    ...
    offer?.acceptedBy !== "skipped"
  )
    return "accepted";

  // Check if offer was skipped
  if (offer?.acceptedBy === "skipped") return "skipped";

  // Default to offered status
  if (offer?.acceptedBy === null || offer?.acceptedBy === undefined)
    return "offered";
