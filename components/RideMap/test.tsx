...
export default function RideMap({
  ...
  const dropoffTestRouteRef = useRef<LocationCoordinates[]>([]);
  ...
  // Effect to hide pickup route when approaching pickup
  useEffect(() => {
    if (currentLocation && pickupLocation && showPickupRoute) {
  ...
  /**
   * Handle when pickup route is ready - set initial car rotation
   */
  const handlePickupRouteReady = (result: any) => {
    if (
      result.coordinates &&
      result.coordinates.length >= 2 &&
      currentLocation
    ) {
      const firstPoint = result.coordinates[0];
      const secondPoint = result.coordinates[1];
      const bearing = calculateBearing(firstPoint, secondPoint);
      setCarRotation(bearing);

      // Store route for testing mode
      testRouteRef.current = result.coordinates;
    }
  };

  /**
   * Handle when dropoff route is ready
   */
  const handleDropoffRouteReady = (result: any) => {
    if (result.coordinates && result.coordinates.length >= 2) {
      dropoffTestRouteRef.current = result.coordinates;
    }
  };
  ...
  const animateNext = (
    ...
    if (index >= route.length - 1) {
      ...
      if (!isDropoff) {
        ...
        const dropoffRoute = interpolateRoute(dropoffTestRouteRef.current);
   ...
   */
  const startTesting = () => {
    if (!currentLocation || testRouteRef.current.length === 0) return;

    setIsTesting(true);

    // Use WebView interpolation logic with more points
    const interpolatedRoute = interpolateRoute(testRouteRef.current);

    if (interpolatedRoute.length < 2) return;

    // Set initial position and rotation
    const firstPoint = interpolatedRoute[0];
    const initialBearing =
      interpolatedRoute.length >= 2
        ? calculateBearing(firstPoint, interpolatedRoute[1])
        : 0;

    setCurrentLocation(firstPoint);
    setCarRotation(initialBearing);
    animatedLatitude.current.setValue(firstPoint.latitude);
    animatedLongitude.current.setValue(firstPoint.longitude);
    animatedRotation.current.setValue(initialBearing);

    // Start chained animations
    animateNext(0, interpolatedRoute, false);
  };
  ...
  return (
    <View style={styles.container}>
      ...
      >
        ...
        {showPickupRoute && GOOGLE_MAPS_API_KEY && (
          <MapViewDirections
            ...
            onReady={handlePickupRouteReady}
            onError={(errorMessage) => {
              log("Directions Error:", errorMessage);
            }}
          />
        )}
        ...
        {GOOGLE_MAPS_API_KEY && (
          <MapViewDirections
            ...
            onReady={handleDropoffRouteReady}
          />
        )}
      ...
      <View style={styles.testingControls}>
        <Button
          ...
          disabled={testRouteRef.current.length === 0}
        >
